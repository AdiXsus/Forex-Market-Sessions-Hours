...
TARGET_CHANNEL_ID = int(
    os.getenv('TARGET_CHANNEL_ID'))  # Dodaj zmienną środowiskową
...
async def check_and_send_message():
  ...
  channel_id = await client.fetch_channel(TARGET_CHANNEL_ID)
  ...
  while not client.is_closed():
    ...
    if now.weekday() == 4 and now.hour == 20 and now.minute == 30:
      ...
      await send_channel_message(channel_id, "🔴 Market closes in 30 minutes.")

    # Sprawdzenie czy jest niedziela i godzina jest odpowiednia
    if now.weekday() == 6:
      if now.hour == 20 and now.minute == 59:
        await change_bot_status("🟢 Open Sydney 23:00"
                                )  # Zmiana statusu o 21:00 w niedzielę

      elif now.hour == 20 and now.minute == 45:
        await send_private_message(target_user, "🟢 Market opens soon.")
        await send_channel_message(channel_id, "🟢 Market opens soon.")
    ...
    if current_time in MESSAGE_SCHEDULE:
      ...
      await send_channel_message(channel_id, message_content)
